...
def find_all_high_fidelity_versions(menu, fidelity_parameter_name,
                                    high_fi_value=0):
    """Filter the menu for high-fi studies only"""

    # Keep only those values with the full fidelity
    menu_high_fi_only = copy.deepcopy(menu)
    tmp = [
        (ii, si) for ii, si in enumerate(menu_high_fi_only["studies"])
        if si["study_parameters"][fidelity_parameter_name]["values"][0] ==
           high_fi_value
    ]

    menu_high_fi_only["studies"] = [tuplei[1] for tuplei in tmp]
    indices_high_fi_only = [tuplei[0] for tuplei in tmp]

    return menu_high_fi_only, indices_high_fi_only


def find_cheaper_version(si, menu, cost_parameter_name,
                         higher_cost_direction=1):
    """Given an expensive study, look for cheaper alternatives"""
    snew = copy.deepcopy(si)
    # Search the menu for any study which matches si while having exactly the
    # same values of all inputs _except_ the cost parameter; return the
    # cheapest of these.
    for si in menu["studies"]:
        try:
            cost_dom = higher_cost_direction * \
                   (snew["study_parameters"][cost_parameter_name]["values"][0] -
                    si["study_parameters"][cost_parameter_name]["values"][0])
            # cost_dom is > 0 if si is less costly than snew
            if cost_dom > 0:
                # Check for study type and parameter match
                if snew["study_type"] != si["study_type"]:
                    break
                if set(snew["study_parameters"].keys()) != \
                        set(si["study_parameters"].keys()):
                    break
                # Must have the same parameter values
                parameter_match = all([
                    snew["study_parameters"][k] == si["study_parameters"][k]
                    for k in snew["study_parameters"].keys()
                    if k != cost_parameter_name
                ])
                if parameter_match:
                    snew = copy.deepcopy(si)
        except KeyError:
...
def main(history_path=None, menu_path=None, studies_path=None,
    ...
    while submitted_this_round < to_submit_this_round and len(list(
        ...
        #       "greedy rule.")
        menu_high_fidelity, indices_menu_hifi = find_all_high_fidelity_versions(
            menu, fidelity_parameter_name="Z", high_fi_value=0)

        s_hifi, f_hat_si = select_eps_greedy(
            [pi for ii, pi in enumerate(predicted_values)
             if ii in indices_menu_hifi],
            menu_high_fidelity, EPSILON)
        si = find_cheaper_version(s_hifi, menu, cost_parameter_name="Z",
                                  higher_cost_direction=-1)
